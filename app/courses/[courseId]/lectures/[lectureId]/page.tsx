...
import { ChevronLeft, ChevronRight, CheckCircle, Clock, FileText, PlayCircle, BookOpen, Menu, X } from "lucide-react"
...
function findLectureContext(course: Course, lectureId: string) {
  ...
  if (currentIndex !== -1) {
    ...
    for (let i = 0; i < course.modules.length; i++) {
      const lectureIdx = course.modules[i].lectures.findIndex((lecture) => lecture.id === lectureId)
      if (lectureIdx !== -1) {
        moduleIndex = i
...
export default function LecturePage() {
  ...
  useEffect(() => {
    async function fetchCourseData() {
      try {
        ...
        console.log("강의 페이지: 모듈 정보 응답", { status: modulesResponse.status })
        ...
        const modulesWithLectures = await Promise.all(
          modulesData.map(async (module: Module) => {
            ...
            if (!lecturesResponse.ok) {
              console.error(`모듈 ${module.id}의 강의 정보를 가져오는 중 오류가 발생했습니다.`)
        ...
        if (user) {
          try {
            const progressResponse = await fetch(`/api/users/progress?courseId=${courseId}`)
            if (progressResponse.ok) {
              const progress = await progressResponse.json()
              progressData = progress.reduce((acc: Record<string, boolean>, item: any) => {
            ...
            console.error("강의 진행 상황을 가져오는 중 오류가 발생했습니다:", error)
          }
        }
        ...
        toast({
          title: "오류 발생",
          description: "강의 데이터를 불러오는 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.",
          variant: "destructive",
        })
  ...
  const toggleLectureCompletion = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("강의 완료 상태를 업데이트하는 중 오류가 발생했습니다.")
      }
      ...
      toast({
        ...
        description: !isCompleted ? "강의를 완료했습니다." : "강의를 미완료 상태로 변경했습니다.",
      })
    } catch (error) {
      ...
      <div className="flex items-center justify-center min-h-screen">
        <Card className="max-w-md w-full">
          <CardContent className="pt-6">
            ...
            <p className="text-muted-foreground mb-4">요청하신 강의를 찾을 수 없습니다.</p>
  ...
  const { currentLecture, prevLecture, nextLecture, moduleIndex } = findLectureContext(course, lectureId)
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <Card className="max-w-md w-full">
          <CardContent className="pt-6">
            <h2 className="text-xl font-bold mb-2">강의를 찾을 수 없습니다</h2>
            <p className="text-muted-foreground mb-4">요청하신 강의를 찾을 수 없습니다. 다른 강의를 선택해주세요.</p>
            <Button asChild className="w-full">
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="bg-background border-b sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between">
          <Link href={`/courses/${courseId}`} className="flex items-center text-sm font-medium hover:underline">
            <ChevronLeft className="h-4 w-4 mr-1" />
            {course.title}
          </Link>
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium">{progressPercentage}% 완료</span>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="max-w-4xl mx-auto px-4 py-8">
            <h1 className="text-2xl md:text-3xl font-bold mb-6">{currentLecture.title}</h1>
            ...
            <div className="space-y-6 mb-8">
              <div>
                ...
                <p className="text-muted-foreground">
                  {currentLecture.description || "이 강의에 대한 설명이 없습니다."}
                </p>
              </div>
            ...
            <div className="flex justify-between mt-8 pt-4 border-t">
              ...
              <Button variant={isCompleted ? "outline" : "default"} onClick={toggleLectureCompletion}>
                {isCompleted ? (
                  <>
        ...
        >
          <div className="p-4">
            ...
            <Accordion type="multiple" defaultValue={[`module-${moduleIndex}`]} className="space-y-2">
              {course.modules.map((module, idx) => (
                <AccordionItem key={module.id} value={`module-${idx}`} className="border rounded-md overflow-hidden">
                      ...
                      {module.lectures.map((lecture) => (
                        <Link
                          ...
                          className={cn(
                            ...
                            lecture.id === lectureId ? "bg-primary text-primary-foreground" : "hover:bg-muted",
                          )}
                        >
                          ...
                          <span className="ml-auto text-xs opacity-70 flex-shrink-0">{lecture.duration || ""}</span>
                        </Link>
                      ))}
                    </div>
                  </AccordionContent>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <div className="flex items-center">
                ...
                <span className="text-sm text-muted-foreground">{currentLecture.duration || "시간 정보 없음"}</span>
              </div>
              <div className="flex items-center">
